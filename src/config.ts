// Application configuration file
// This file serves as the single source of truth for Appwrite and Gemini credentials
// to ensure stable operation across client-side deployments without environment variable dependencies.

export const DEFAULT_APPWRITE_ENDPOINT = 'https://cloud.appwrite.io/v1';
export const DEFAULT_APPWRITE_PROJECT_ID = '6a5c48fb00236c305a1c';
export const DEFAULT_APPWRITE_DATABASE_ID = '6a5c43ce003718f2ed71';
export const DEFAULT_APPWRITE_STORAGE_BUCKET_ID = '6a5c4e1c001a2be6ae0d';

// Default Gemini API key used directly on client-side requests
export const DEFAULT_GEMINI_KEY = 'AQ.Ab8RN6IiduFA80UTJ8fCjdR0KxHCgAZQ-Gi7W_H0C56GBD7RXw';
